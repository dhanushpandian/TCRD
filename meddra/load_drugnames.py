...
tsv_file_path = "data/drug_names.tsv"  # Adjust the path to your TSV file
df = pd.read_csv(tsv_file_path, sep='\t', header=None, names=['drug_id', 'drug_name'])
...
def insert_data(cursor, table_name, data_frame):
    for index, row in data_frame.iterrows():
        # Skip rows where 'drug_id' or 'drug_name' is missing
        if pd.isnull(row['drug_id']) or pd.isnull(row['drug_name']):
            print(f"Skipping row {index + 1} due to missing data.")
            continue
        
        insert_query = f"""
        INSERT INTO {table_name} (drug_id, drug_name) 
        VALUES (%s, %s)
        ON DUPLICATE KEY UPDATE 
            drug_name = VALUES(drug_name)
        """
        try:
            cursor.execute(insert_query, (row['drug_id'], row['drug_name']))
            conn.commit()
...
insert_data(cursor, "drug_names", df)  # Replace "drug_table" with your actual table name
